# %%
""" Check the error of each layer across inference"""
import os
import pandas as pd
import torch
...
def test_model(
    ...
    for itr in range(num_iters):
        ...
        # save layer errors
        for n in range(model.num_layers):
            errors[n][:, itr, :] = model.errs[n + 1].cpu().numpy()

    return errors


# %%
# define function to extract layerwise errors across inference steps
...
# Save the plot
fig.savefig(
    "../../results/exp_2_norm_tests/error_normal_twin_0_49_inf.png",
    bbox_inches="tight",
    dpi=DPI,
)
