...
def run_exp(output_dir: str, seeds: List[int]):
    """Run difference check (amort better than hybrid)"""

    prog_folder = os.path.join(output_dir, "diff_check_mnist_emnist/progenitor")
    ...
    for seed in seeds:
        ...
        cfg.exp.num_epochs = 4  # number of epochs
        cfg.exp.test_every = 1  # test every 5 epochs
        cfg.exp.save_models = True  # save model pickle on each epoch

        # change the dataset
        cfg.data.dataset = "EMNIST_c-MNIST"
        # select only some classes within the dataset
        cfg.data.dataset_classes = list(range(10))
        cfg.model.n_amort_nets = 1

        # configure network architecture
        cfg.model.nodes = [10, 500, 500, 784]
        cfg.model.amort_nodes = [784, 500, 500, 10]
