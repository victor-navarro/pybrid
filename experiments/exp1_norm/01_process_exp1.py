...
def process_exp(output_dir: str, seeds: List[int]):
    ...
    if GET_PROBS:
        for folder in seed_folders:
            # find model files
            mfs, epochs = utils.get_model_files(folder)
            for mf, epoch in tqdm(zip(mfs, epochs), desc="Getting label probabilities"):
                # set csv path
                csv_path = os.path.join(folder, f"label_probabilities/{epoch}.csv")

                # get ranks
                rank_df = post.get_label_probability(
                    folder, pkl_name=mf, test_iters=TEST_ITERS
                )
                # make directory
                os.makedirs(os.path.dirname(csv_path), exist_ok=True)
