...
def plot_exp(seeds: List[int]):
    ...
    for twin in ["normal_twin", "swapped_twin"]:
        ...
        all_data = all_data.groupby(["class", "iteration", "network", "label", "epoch"]).agg(
            mean=("label_avg", "mean"),
            sem=("label_avg", "sem"),
        ).reset_index()

        # create figure
        fig, ax = plt.subplots(len(EPOCHS)*2, 2, figsize=FIGSIZE, layout="constrained", height_ratios= [2,1]* len(EPOCHS))
        for ei, epoch in enumerate(EPOCHS):
            for ni, network in enumerate(["hybrid", "pc"]):
                # set ylabel if first column
                if ni == 0:
                    ax[ei*2, ni].set_ylabel(f"Batch {(epoch+1) * 56}\nProbability")
                # set x label if last row
                if ei == len(EPOCHS) - 1:
                    ax[ei*2, ni].set_xlabel("Iteration")
                # set title for each subplot
                if ei == 0:
                    ax[ei*2, ni].set_title(labels[network])
                # set y limits
                ax[ei*2, ni].set_ylim(0, 1)
                # select data for the epoch and network
                pdat = all_data[(all_data["epoch"] == epoch) & (all_data["network"] == network)]
                # plot the data
                for label, group in pdat.groupby("label"):
                    ...
                    ax[ei*2, ni].plot(
                        ...
                        group["mean"],
                        label=lab,
                        color=colours[lab],  # default to grey if not found
                        zorder=zorder
                    )
                ...
                # now we embed the label reconstruction images
                img_path = os.path.join(f"plots/extra/iteration_images/from_label/{twin.replace('_twin', '')}/epoch_{epoch+1}/{network}_imgs.png")
                img = plt.imread(img_path)
                ax[ei*2+1, ni].imshow(img, cmap="gray", aspect="auto")
                ax[ei*2+1, ni].axis("off")

        hs, ls = ax[2,1].get_legend_handles_labels()
        by_label = dict(zip(ls, hs))
        # reorder so ot her appears last
        by_label = {k: by_label[k] for k in sorted(by_label, key=lambda x: x == "Other")}
        ax[2,1].legend(by_label.values(), by_label.keys(), 
                       title="Label",
                       loc="upper left", bbox_to_anchor=(1, 1), fontsize=8)
        plot_path = f"plots/exp_1_norm/label_probabilities_{twin}_{CLASS}.png"
