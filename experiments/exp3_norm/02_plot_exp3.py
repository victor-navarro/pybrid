""" Creates decent plots for experiment 3 """
...
def plot_exp_3(seeds: List[int]):
    ...
    for s in seeds:
        ...
        csv_files = glob.glob(f"results/exp_3_norm/progenitor/{s}/label_accuracy/*.csv")
    ...
    acc_data.to_csv("results/exp_3_norm/progenitor_accuracy.csv", index=False)
    ...
    os.makedirs("plots/exp_3_norm/", exist_ok=True)
    fig.savefig(
        "plots/exp_3_norm/progenitor_accuracy.png", bbox_inches="tight", dpi=300
    )
    ...
    for s in seeds:
        # get all csv files in the folder
        csv_files = glob.glob(f"results/exp_3_norm/progenitor/{s}/reconstruction/*.csv")
        # load the data and append epoch (leading digits in filename) as a column
        for f in csv_files:
            epoch = int(os.path.basename(f).split(".")[0])
            rec_data = pd.concat(
    ...
    rec_data.to_csv("results/exp_3_norm/progenitor_reconstruction.csv", index=False)
    ...
    ax.set_yticks(np.arange(0, 1.25, 0.25))
    fig.savefig(
        "plots/exp_3_norm/progenitor_reconstruction.png", bbox_inches="tight", dpi=DPI
    )
    ...
    for twin in ["normal", "swapped"]:
        for s in seeds:
            csv_files = glob.glob(
                f"results/exp_3_norm/{twin}_twin/{s}/label_accuracy/*.csv"
            )
            for f in csv_files:
                epoch = int(os.path.basename(f).split(".")[0])
    ...
    twin_acc_data.to_csv("results/exp_3_norm/twin_accuracy.csv", index=False)
    # now make the plot of overall accuracy on the last iteration step
    # select only the last iteration per class/network
    ftwin_acc_data = (
    ...
    fig.savefig("plots/exp_3_norm/twin_accuracy.png", bbox_inches="tight", dpi=DPI)
    ...
    for twin in ["normal", "swapped"]:
        for s in seeds:
            csv_files = glob.glob(
                f"results/exp_3_norm/{twin}_twin/{s}/reconstruction/*.csv"
            )
            for f in csv_files:
                epoch = int(os.path.basename(f).split(".")[0])
    ...
    twin_rec_data.to_csv("results/exp_3_norm/twin_reconstruction.csv", index=False)
    # now make the plot of overall rec_error
    # get average rec_error across classes, per batch
    ftwin_rec_data = (
    ...
    ax[i].legend(loc="center left", bbox_to_anchor=(1, 0.5))
    fig.savefig(
        "plots/exp_3_norm/twin_reconstruction.png", bbox_inches="tight", dpi=DPI
    )


if __name__ == "__main__":
    plot_exp_3(list(range(8)))
