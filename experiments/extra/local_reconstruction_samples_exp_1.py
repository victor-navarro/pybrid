...
def remake_images():
    ...
    for epoch in [0, 99]:
        ...
        preds["pc"].append(utils.postprocess_prediction(model.preds[-1]))
        ...
        preds["amort"].append(utils.postprocess_prediction(model.preds[-1]))
    ...
    for twin in ["normal", "swapped"]:
        ...
        for e, epoch in enumerate([0, 49]):
            ...
            img_path = f"plots/exp_1_norm/{twin}_{epoch}_mosaic_local.png"
            Image.fromarray(mosaic).save(img_path)
