...
def iteration_images(model_pkl: str, model_cfg_path: str, out_folder: str,
                     classes: Optional[List[int]] = None,
                     steps: Optional[List[int]] = None, from_labels: bool = False, 
                     ncol: int = 1, nrow: int = 12):
    ...
    # get images
    if from_labels:
        hybrid_imgs, pc_imgs, amort_imgs, label_imgs, imgs = _get_label_images(model, img_set)
    else:
    ...
    for i in steps:
        pc_mos.append(utils.make_mosaic(
            np.array(pc_imgs[i]),
            nrow=nrow,
            ncol=ncol,
        ))
    ...
    hybrid_mos = []
    for i in steps:
        hybrid_mos.append(utils.make_mosaic(
...
if __name__ == "__main__":
    ...
    # FROM LABELS
    # BY BATCHES
    base_folders = {"normal" : "results/exp_1_norm_e1/normal_twin/0/", 
                    "swapped" : "results/exp_1_norm_e1/swapped_twin/0/"}
    model_pkls = {"0": "model_0.pkl", "1": "model_1.pkl", "15": "model_15.pkl", "29": "model_29.pkl", "43": "model_43.pkl", "56": "final_model.pkl"}
    for model_name, base_folder in base_folders.items():
        for batch, model_pkl in model_pkls.items():
            # set the model pkl
            model_pkl = os.path.join(base_folder, model_pkl)
            # set the config path
            cfg_path = os.path.join(base_folder, "config.json")
            # set the output folder
            out_folder = os.path.join("plots/extra/iteration_images/from_label", model_name, f"batch_{batch}")
            iteration_images(model_pkl, cfg_path, out_folder, steps=steps, classes=classes, from_labels=True)
    
    # BY EPOCHS
    base_folders = {"normal" : "results/exp_1_norm/normal_twin/0/", 
                    "swapped" : "results/exp_1_norm/swapped_twin/0/"}
    model_pkls = {"1": "model_0.pkl", "10": "model_9.pkl", "20": "model_19.pkl", "30": "model_29.pkl", "40": "model_39.pkl", "50": "model_49.pkl"}
    for model_name, base_folder in base_folders.items():
        for batch, model_pkl in model_pkls.items():
            # set the model pkl
            model_pkl = os.path.join(base_folder, model_pkl)
            ...
            cfg_path = os.path.join(base_folder, "config.json")
            # set the output folder
            out_folder = os.path.join("plots/extra/iteration_images/from_label", model_name, f"epoch_{batch}")
            iteration_images(model_pkl, cfg_path, out_folder, steps=steps, classes=classes,from_labels=True)
    ...
    for model_name, base_folder in base_folders.items():
        for batch, model_pkl in model_pkls.items():
            ...
            cfg_path = os.path.join(base_folder, "config.json")
            # set the output folder
            out_folder = os.path.join("plots/extra/iteration_images/local", model_name, f"batch_{batch}")
            iteration_images(model_pkl, cfg_path, out_folder, steps=steps, classes=classes,from_labels=False)
    ...
    for model_name, base_folder in base_folders.items():
        for batch, model_pkl in model_pkls.items():
            ...
            cfg_path = os.path.join(base_folder, "config.json")
            # set the output folder
            out_folder = os.path.join("plots/extra/iteration_images/local", model_name, f"epoch_{batch}")
            iteration_images(model_pkl, cfg_path, out_folder, steps=steps, classes=classes,from_labels=False)