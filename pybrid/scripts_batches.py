...
# NOTE (VN): This is a small modification on scripts,
# to save model outputs on a batch by batch basis.
...
def main(cfg: DefaultConfig, save_batches: bool = False):
            ...
            for batch_id, (img_batch, label_batch, context_batch) in enumerate(
                ...
                if save_batches:
                    if cfg.exp.gen_infer_images:
                        ...
                        img_name = cfg.exp.img_dir + f"/infer_img_{total_batches}.png"
                        # save
                        utils.save_img(img_preds, img_name)

                    if cfg.exp.gen_label_images:
                        logging.info(
                            "Generating image @ %s/%d.png",
                            cfg.exp.img_dir,
                            total_batches,
                        )
                        # now do a backward pass with the label predictions
                        ...
                        img_name = cfg.exp.img_dir + f"/label_img_{total_batches}.png"
                        # save
                        utils.save_img(img_preds, img_name)
                    ...
                    if cfg.exp.save_models:
                        ...
                        logging.info(
                            "Saving model @ %s/model_%d.pkl",
                            cfg.exp.log_dir,
                            total_batches,
                        )
                        utils.save_pkl(
                            model,
                            os.path.join(cfg.exp.log_dir, f"model_{total_batches}.pkl"),
                        )
