...
class HybridModel(BaseModel):
    ...
    def forward_layer(self, max_layer, val):
        """Forward pass through the amortization network up to the max_layer-th layer"""
        for layer in self.amort_layers[:max_layer]:
            val = layer.forward(val)
        return val

    def backward_layer(self, starting_layer, val):
        """Backward pass through the inferential network starting from the starting_layer-th layer"""
    ...
    def train_batch(
        ...
        num_iter, avg_errs = self.train_updates(
            num_iters, fixed_preds=fixed_preds, thresh=thresh
        )
        self.update_grads()
        if use_amort:
