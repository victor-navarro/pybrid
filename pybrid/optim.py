...
class Optimizer(object):
    ...
    def decay_weights(self, param):
        if self.weight_decay is not None:
            param.grad["weights"] = (
                param.grad["weights"] - self.weight_decay * param.weights
            )
    ...
    def step(self, curr_epoch=None, curr_batch=None, n_batches=None, batch_size=None):
        with torch.no_grad():
            ...
            for p, param in enumerate(self._params):
                ...
                self.c_w[p] = (
                    self.beta_1 * self.c_w[p]
                    + (1 - self.beta_1) * param.grad["weights"]
                )
                self.v_w[p] = (
                    self.beta_2 * self.v_w[p]
                    + (1 - self.beta_2) * param.grad["weights"] ** 2
                )
                delta_w = (
                ...
                if param.use_bias:
                    self.c_b[p] = (
                        self.beta_1 * self.c_b[p]
                        + (1 - self.beta_1) * param.grad["bias"]
                    )
                    self.v_b[p] = (
                        self.beta_2 * self.v_b[p]
                        + (1 - self.beta_2) * param.grad["bias"] ** 2
                    )
                    delta_b = (
                        np.sqrt(1 - self.beta_2**t)
                        * self.c_b[p]
